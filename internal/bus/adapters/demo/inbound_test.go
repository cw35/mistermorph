...
func TestInboundAdapterInboxDedupe(t *testing.T) {
	...
	adapter, err := NewInboundAdapter(InboundOptions{
		Bus:     b,
		Store:   store,
		Channel: "telegram",
	...
	accepted, err := adapter.HandleInbound(ctx, "1001", msg)
	if err != nil {
		t.Fatalf("HandleInbound(first) error = %v", err)
	}
	if !accepted {
		t.Fatalf("HandleInbound(first) accepted=false, want true")
	}
	select {
	case <-done:
	...
	if accepted {
		t.Fatalf("HandleInbound(second) accepted=true, want false")
	}

	record, ok, err := store.GetBusInboxRecord(ctx, contacts.ChannelTelegram, "1001")
	if err != nil {
...
func TestInboundAdapterBoundaryValidation(t *testing.T) {
	ctx := context.Background()
	store := contacts.NewFileStore(filepath.Join(t.TempDir(), "contacts"))
	if err := store.Ensure(ctx); err != nil {
		t.Fatalf("Ensure() error = %v", err)
	...
	msg := validInboundMessage(t)
	msg.IdempotencyKey = ""
	if _, err := adapter.HandleInbound(ctx, "1002", msg); err == nil {
		t.Fatalf("HandleInbound() expected validation error")
	}
	if _, ok, err := store.GetBusInboxRecord(ctx, contacts.ChannelTelegram, "1002"); err != nil {
		t.Fatalf("GetBusInboxRecord() error = %v", err)
	} else if ok {

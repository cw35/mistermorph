...
- 🤝 **Mesh Agent Exchange Protocol（MAEP）**: 複数の Agent を運用し、相互にメッセージをやり取りしたい場合に MAEP を使えます。信頼状態と監査トレイルを備えた P2P プロトコルです（[../feat/feat_20260206_maep.md](../feat/feat_20260206_maep.md) を参照、WIP）。
...
デフォルトでは `run` は `skills.mode=smart` を使うため、どのスキルを読み込むかは Agent が自動で判断します（`$SkillName` を明示しなくても動作）。
